...
>>>
# 此命令默认将所有的属性输出，如果想获取单个属性的值，只需要在后面跟上属性的名称即可：
    #列入：    psutil.cpu_times().user    #获取CPU 用户级别程序使用的 CPU 时间。
该命名元组中包含了如下属性：        #各个属性对应的单位是秒（s）
user:     CPU 用户级别程序使用的 CPU 时间。
nice:     CPU 低优先级用户级别程序使用的 CPU 时间。
system:     CPU 内核级别程序使用的 CPU 时间。
idle:     CPU 空闲时间。
iowait:     CPU 等待输入输出完成的 CPU 时间。
irq:     CPU 中断处理程序使用的 CPU 时间。
softirq:     软件中断处理程序使用的 CPU 时间。
steal:     虚拟化环境中虚拟 CPU 被 hypervisor 转移到另一个虚拟机上的 CPU 时间。
guest:     在虚拟机中运行虚拟 CPU 使用的 CPU 时间。
```
...
如下先获取分区信息，然后看下根分区的使用情况:[](javascript:void(0);)
...
## disk_usage("C:\\")   查看某个磁盘使用情况

```text
import psutil
print(psutil.disk_usage("C:\\"))        #参数是传入挂载点，windows中，就是盘符
...
pprint(psutil.net_if_addrs())        #返回对象是一个字典
"""
...
pprint(psutil.pids())
"""
[0,4,144,512,36,632,640,664,696,768,776,
 ...
 ...
 ...
]
...
## Process(pid=0) 根据 pid 获取一个进程对应的 Process 对象
