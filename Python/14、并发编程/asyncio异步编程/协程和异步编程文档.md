...
loop = asyncio.get_event_loop()				#生产事循环
loop.run_until_complete(asyncio.wait(tasks))
```

注意：基于asyncio模块实现的协程比之前的要更厉害，因为他的内部还集成了遇到IO耗时操作自动切花的功能。

### 1.3.1 使用yield 自定义IO 模拟asyncio.sleep()

> yield from 后面必须跟一个迭代器对象，所以我们只需要自定义出一个时间可控的，用于阻塞的迭代器对象即可。
>
> myIO 内部使用yiel from sleepIO(),这样的话可以不考虑sleepIO() 的内部实现。只需要关心它是用于实现IO的就行了。
>
> &#x9;\-至于sleepIO() 怎么实现，或者后期有改动都是sleepIO的事情。 如果直接将逻辑写在myIO里面的话，耦合度就太高了。
...
def sleepIO(sec):
    for item in range(0,sec):
        yield               #time.sleep(1) 一定要放在yield的下面，如果yield下面如果没有任何代码，就跟没写yield似的，不生效。
        time.sleep(1)



def myIO(sec):
    yield from sleepIO(sec)
...
def fun1():
    print("fun1 start")
    # yield from asyncio.sleep(3)        #不能替换为time.sleep(1)
    #因为yield from 后面要跟一个迭代对象，time.sleep(1)  不是
    yield from  myIO(3)
    print("fun1 end")
...
注意：`asyncio.wait` 源码内部会对列表中的每个协程执行ensure\_future从而封装为Task对象，所以在和wait配合使用时task\_list的值为`[func(),func()]` 也是可以的。
